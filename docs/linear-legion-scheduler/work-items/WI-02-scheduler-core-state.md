...
> **状态**: 已完成为 `scheduler/` 独立 npm project 中的 SQLite-backed scheduler core skeleton；后续 WI-03 / WI-04 / WI-05 应通过 repository / outbox 边界接入，不直接绕过 Scheduler DB truth。
...
- `scheduler/` 独立 npm project 形式的 repo-local development / test setup。
