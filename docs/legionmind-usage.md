...
推荐阅读顺序：`.legion/tasks/<task-id>/plan.md` -> `.legion/tasks/<task-id>/docs/rfc.md`（若存在）-> `.legion/tasks/<task-id>/context.md` / `.legion/tasks/<task-id>/tasks.md`。
...
1. 运行 `/legion`。
2. 先生成 `.legion/tasks/<task-id>/plan.md`（问题、验收、假设、约束、风险、Scope/Design Index/Phase Map）。
...
2. 生成设计产物（`plan`、`research`、`rfc`、`review-rfc`、`report-walkthrough`、`pr-body`；其中 `plan.md` 在任务根目录，其余在 `.legion/tasks/<task-id>/docs/`）。
