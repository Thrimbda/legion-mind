...
- 工作流入口：`legion-workflow`，负责接管判定、任务恢复与阶段路由。
...
- 持久化状态：`.legion/tasks/**` 保存原始任务文档（`plan.md` + `log.md` + `tasks.md`），`.legion/wiki/**` 统一保存跨任务综合知识（决策、模式、维护债务、任务摘要）；本地 CLI 只围绕这些文件工作，不再维护额外注册表或审计账本。

这里所说的 active task，只指当前请求明确恢复并继续推进的 `.legion/tasks/<task-id>/` 任务目录，不是仓库级持久单例。
...
适合低风险或中风险的大多数任务，主干是：接管判定 -> 恢复当前请求明确指向的既有任务，或进入 `brainstorm` -> 设计简述 / RFC -> 实现 -> 测试 -> 评审 -> 报告 -> 收口写回。

### 重型设计模式

用于史诗级任务或高风险任务。只做重设计，不写生产代码，主干是：接管判定 -> 恢复当前请求明确指向的既有任务，或进入 `brainstorm` -> 调研 / RFC -> RFC 对抗审查 -> 面向评审者的摘要 -> 收口写回。

### 已批准设计后的实现续跑

用于已有稳定契约且设计已经批准后的实现阶段，主干是：显式恢复该既有任务 -> `engineer` -> `verify-change` -> `review-change` -> `report-walkthrough` -> 收口写回。
...
这也不是子代理派生真源。首次进入新仓库时，先完成安装并初始化 `.legion/tasks/`；wiki 页面由后续 writeback 按需建立，不假定 `init` 预建 skeleton。

## 5. 典型流程

### 5.1 低风险默认流（推荐）

1. 从 `legion-workflow` 入口开始，让它先判断是否接管、恢复当前请求明确指向的既有任务，或进入 `brainstorm`。
...
5. 设计 PR 合并后，显式恢复该既有任务进入实现阶段，按“已批准设计后的实现续跑”继续。

补充标签语义：

- `plan-only`：只做设计与计划，不进入实现。
- `continue`：口头简写而已，本质仍是显式恢复某个既有 `.legion/tasks/<task-id>/` 任务后继续下一阶段；它不代表任何持久化 current-task 机制。
...
- 任务命令失败：先检查 `.legion/` 是否存在，再确认命令是否显式传入了 `--task-id` 或 JSON `taskId`。
