...
4. Each WI must use an independent Legion task and delivery branch. Update an existing open PR when it is the current delivery surface. Do not autonomously create a closeout, publish-result, deploy-result, or wiki-only PR solely to record terminal lifecycle facts. If terminal follow-up needs repository changes, stop and wait for explicit user authorization; that authorization may allow a new PR.
5. PR created is not completion. Finish repository evidence and wiki writeback before the current PR becomes terminal; merged/closed, cleanup, refresh, publish/deploy, and final reporting are external-only Scheduler observations.
...
export function parseWorkerResultBlock(output: string): WorkerResultBlock {
  ...
  if (typeof parsed.prUrl === 'string') {
    let parsedPrUrl: URL;
    try {
      parsedPrUrl = new URL(parsed.prUrl.trim());
    } catch {
      throw new Error('Worker result prUrl must be a supported HTTPS pull request URL.');
    }
    const segments = parsedPrUrl.pathname.split('/').filter(Boolean);
    const number = Number(segments[3]);
    if (parsedPrUrl.protocol !== 'https:' || parsedPrUrl.username || parsedPrUrl.password || segments.length !== 4 || segments[2] !== 'pull' || !segments[0] || !segments[1] || !Number.isInteger(number) || number <= 0) {
      throw new Error('Worker result prUrl must be a supported HTTPS pull request URL.');
    }
  }
