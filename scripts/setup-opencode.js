...
const COMMANDS = ['install', 'verify', 'rollback', 'uninstall']         ;
...
Examples:
  ...
  npm install -g lgmind && lgmind install --scope global
...
function parseArgs(argv          )             {
  ...
  const command = (commandArg                       ) ?? 'install';
  ...
  const strategyValue = (getValue('--strategy') ?? 'copy')            ;
...
function managedRootPaths(opts            )           {
  return [
...
function loadManagedStateForVerify(opts            )                     {
...
function runInstall(opts            , runId        , reporter          )               {
  ...
  const managedState = loadJsonOrDefault              (managedFilePath, emptyManagedState());
  const backupIndex = loadJsonOrDefault             (backupIndexPath, emptyBackupIndex());
...
function runUninstall(opts            , runId        , reporter          )               {
  const stateDir = join(opts.configDir, MANAGED_DIR_NAME);
  const managedFilePath = join(stateDir, MANAGED_FILES_FILE);
  const managedState = loadJsonOrDefault              (managedFilePath, emptyManagedState());
