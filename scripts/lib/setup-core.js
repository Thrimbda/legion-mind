...
export class Reporter {
  ...
  emit(code        , phase        , checkId        , target        , hint        ) {
    const payload = {
...
export function formatInstallStateSummary(state                  , runtime        )         {
...
export function resolvedLinkTarget(path        )         {
  return resolve(dirname(path), readlinkSync(path));
}

export function sourceFingerprint(path        , strategy          )         {
  if (strategy === 'symlink') {
...
export function canonicalDirectoryPath(path        )         {
  const absolute = resolve(path);
...
export function createManagedRootSet(roots          )                 {
...
export function targetWithinManagedRoots(path        , roots                , options                                                                                     )          {
...
export function writeJsonAtomic(path        , data         , dryRun         ) {
  if (dryRun) {
    return;
  }
...
export function loadJsonOrDefault   (path        , fallback   )    {
  if (!existsSync(path)) {
    return fallback;
  }
  try {
...
export function validateManagedStateFile(path        , allowedActions                     )                     {
  if (!existsSync(path)) {
...
export function validateBackupIndexFile(path        )                    {
  if (!existsSync(path)) {
    return { kind: 'missing', path };
  }
  try {
    ...
    return { kind: 'ok', path, index: parsed                };
  } catch (error) {
    return { kind: 'invalid', path, error: error instanceof Error ? error.message : 'invalid json' };
  }
}
...
export function emptyBackupIndex()              {
  return { version: 1, updatedAt: new Date(0).toISOString(), backups: [] };
}

export function backupPathFor(targetPath        , backupId        )         {
  return `${targetPath}.backup-${backupId}`;
}

export function removeTarget(path        , dryRun         ) {
...
export function ensureParentDir(targetPath        , dryRun         ) {
  if (!dryRun) mkdirSync(dirname(targetPath), { recursive: true });
}

export function safeOverwriteHint(reason        )         {
  switch (reason) {
...
export function canOverwrite(targetPath        , sourceFingerprintValue        , strategy          , managedState              , force         )                                       {
...
export function syncOneFileCore(item          , ctx                  , managedState              , backupBatch             , reporter          , counters                                                     ) {
...
export function verifyStrictItemCore(item          , managedState              , reporter          , projectRoot        , scriptName        )         {
...
export function rollbackCore(input                                                                                                                                                         )                                                                                                            {
...
export function uninstallCore(input                                                                           )                                                                                 {
  let removed = 0;
  let skipped = 0;
