...
- Gate：MVP implementation-ready 必须要求 `contract:stable`；Linear-side contract / scheduling policy 的当前可执行文档是 `docs/linear-legion-scheduler/linear-wi-contract-policy.md`，SQLite-backed scheduler core / durable state 的当前可执行文档是 `docs/linear-legion-scheduler/scheduler-core-sqlite.md`，运行时代码当前放在 `scheduler/` 独立 npm project 中。WI-02 当前使用 SQLite 作为本地 durable-state prototype；以后迁移 Postgres / workflow engine 时必须保留同一 repository、transaction、lock、event、outbox 和 terminal semantics。downstream unlock 不能只看 Linear Done、AgentSession complete 或 PR open，必须通过 `isBlockerSatisfied()`。`Done` 只表示 `run_terminal_success`；`canceled`、`abandoned`、`closed-unmerged`、`superseded` 等 terminal non-success 默认不释放 downstream。PR-backed success 还必须包含 Legion evidence PASS、PR merged、checks/review satisfied、worktree cleanup 和 main refresh；缺 cleanup / refresh 时进入 `lifecycle_blocked`。PR URL 也不是 Legion 完成证据，scheduler 需要 evidence verifier 来拒绝缺 `plan.md`、`review-rfc` / `review-change`、`report-walkthrough`、wiki writeback 等证据的结果。不要在首版引入 OpenClaw / Codex / custom runtime adapter。
