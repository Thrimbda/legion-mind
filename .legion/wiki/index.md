...
- `report-walkthrough` 当前模式是 HTML-first、基于有效证据的 reviewer handoff 协议：`docs/report-walkthrough.html` 是主 reviewer artifact，Markdown 是 compact source / fallback，PR body 是 PR 输入；见 `patterns.md`、`tasks/harden-report-walkthrough.md` 与 `tasks/html-first-report-walkthrough.md`，schema 真源仍是 `skills/report-walkthrough/SKILL.md`。
