...
- `setup-opencode` 现在准备作为 npm CLI 发布：package/bin 目标为 `setup-opencode`，入口是 portable JS wrapper，README 使用 `npx setup-opencode@latest` 叙事，package dry-run 是默认 release-readiness 验证面之一。见 `patterns.md` 与 `tasks/setup-opencode-npm-cli.md`。
