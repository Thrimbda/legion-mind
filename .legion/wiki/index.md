...
- OpenCode installer 的 npm CLI 发布面已收敛为 package `lgmind`：primary bin `lgmind`，alias bin `setup-opencode`，入口是 portable JS wrapper，README 使用 `npx lgmind@latest` 叙事；package dry-run、registry name/auth 检查与 repo-local npm cache 是 release-readiness 验证面。见 `patterns.md`、`tasks/setup-opencode-npm-cli.md` 与 `tasks/publish-lgmind-npm.md`。
