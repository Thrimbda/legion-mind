...
- Linear + Legion 自动调度器的当前设计提案见 `tasks/linear-legion-scheduler-rfc.md`、`tasks/linear-legion-scheduler-wi-01.md`、`tasks/linear-legion-scheduler-wi-02.md`、`tasks/lock-scheduler-worker-opencode.md`、`tasks/amend-linear-native-scheduler-rfc.md` 与 `docs/linear-legion-scheduler/`；WI-01 的 Linear-side contract / scheduling policy 已落在 `docs/linear-legion-scheduler/linear-wi-contract-policy.md`，WI-02 的 SQLite scheduler core / durable state 已落在 `docs/linear-legion-scheduler/scheduler-core-sqlite.md`。可复用集成模式是“外部调度器只编排，不替代 Legion 阶段链”，首版 worker runtime 锁定 OpenCode，Scheduler DB 是 machine truth，Linear Native Agent layer 只做 presentation/control plane，`Done` 只代表 `run_terminal_success`。
