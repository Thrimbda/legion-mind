...
- Linear + Legion 自动调度器的当前设计提案见 `tasks/linear-legion-scheduler-rfc.md`、`tasks/lock-scheduler-worker-opencode.md` 与 `docs/linear-legion-scheduler/`；可复用集成模式是“外部调度器只编排，不替代 Legion 阶段链”，且首版 worker runtime 锁定 OpenCode。
