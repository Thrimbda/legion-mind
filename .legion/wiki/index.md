...
- Git worktree + PR lifecycle envelope 当前不设 task 级 PR 数量或永久 identity 限制；真正的硬边界是 terminal 状态事实外部化，workflow 不自动创建 closeout/status-writeback PR，用户明确授权后仍可开始新的 PR 交付。见 `decisions.md`、`patterns.md`、`tasks/remove-pr-quota-enforcement-v1.md`、历史页 `tasks/enforce-single-pr-lifecycle-v1.md` 与 `tasks/add-git-worktree-pr-envelope.md`，执行细节真源是 `skills/git-worktree-pr/SKILL.md`。
