...
- Linear + Legion 自动调度器的当前设计提案见 `tasks/linear-legion-scheduler-rfc.md`、`tasks/lock-scheduler-worker-opencode.md`、`tasks/amend-linear-native-scheduler-rfc.md` 与 `docs/linear-legion-scheduler/`；可复用集成模式是“外部调度器只编排，不替代 Legion 阶段链”，首版 worker runtime 锁定 OpenCode，Linear Native Agent layer 只做 presentation/control plane，`Done` 只代表 `run_terminal_success`。
