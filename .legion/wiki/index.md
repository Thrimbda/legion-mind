...
- `report-walkthrough` 当前模式是 HTML-first、基于有效证据的 reviewer handoff 协议：`docs/report-walkthrough.html` 是主 reviewer artifact，Markdown 是 compact source / fallback，PR body 是 PR 输入；PR-backed HTML artifact 完成后默认交给 `pr-html-render` 形成 rendered preview path，或记录 explicit render bypass / blocker。见 `patterns.md`、`tasks/harden-report-walkthrough.md`、`tasks/html-first-report-walkthrough.md` 与 `tasks/pr-html-render-skill.md`；schema 真源仍是 `skills/report-walkthrough/SKILL.md` 与 `skills/pr-html-render/SKILL.md`。
