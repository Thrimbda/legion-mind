...
description: 为 Legion 修改型开发任务提供单一 PR identity、隔离 worktree、checks/review、终态、cleanup 与主工作区刷新外壳。
---
...
- 每个 `taskId` 的 PR cardinality 固定为 `0..1`。首次创建或发现 PR 后绑定 owner/repo/number identity；相同 PR 可幂等更新，不同 PR、terminal 后 replacement 或任何第二 PR一律 fail closed。
...
5. **Push/PR**：先查询 task/branch 的既有 PR。没有 identity 时创建唯一 squash PR；已有 open PR 时只更新同一开发分支与同一 PR；已有 merged/closed PR 时不得再创建、替换或更新 PR。PR 链接适用的 plan/RFC/test/review/walkthrough/wiki 证据与 disposition。
...
8. **Terminal**：merged 是成功；closed/confirmed abandoned 是非成功终态。terminal 立即进入 external-only：后续原因、影响、publish/deploy、cleanup 与 refresh 只写外部状态/最终交接，不提交仓库；blocked handoff 不是终态。
...
repo 内 task/wiki 只需在唯一 PR 中达到 `delivery-ready`。完成判断来自唯一 PR、GitHub/checks/review 与本地 lifecycle 的直接观察；不得为了把仓库状态改成 `completed` 再开 closeout、publish-result、deploy-result、wiki-only 或其他第二 PR。

## 阻塞交接与记录

权限、平台、checks/review 或保护规则阻塞时在外部交接记录：blocker，已完成/未完成动作，base、branch、worktree、唯一 PR URL/state，checks/review state，cleanup/refresh state，下一步 owner 与恢复条件；不得称 done。唯一 PR terminal 后不得为记录 blocker 或恢复状态修改仓库；需要代码变更时停止当前 task。

禁止强推覆盖他人工作、使用非 squash 合并、把 lifecycle 写成 Legion 新模式、替换已绑定 PR identity，或为同一 task 创建第二 PR。pre-terminal 仓库证据留在 repo；post-terminal 事实属于外部 lifecycle。
