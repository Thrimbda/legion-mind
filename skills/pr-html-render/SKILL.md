...
- 默认用中文回答 render 方案、风险说明、配置提醒和给用户的最小输出。
- render handoff、PR comment 文案、preview 说明、安全 caveat 等人类阅读型文档产物默认使用中文。
- HTML 文件内容、URL、GitHub Actions 字段、workflow key、环境变量、命令、错误原文和平台术语保持原文；只有用户明确要求时才翻译展示性 copy。
