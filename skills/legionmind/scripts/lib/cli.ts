...
export function createTask(ctx: CliContext, input: Record<string, unknown>, fromProposal?: Proposal) {
  validateFields(input, ['name', 'goal', 'points', 'scope', 'phases', 'rationale']);
  const config = loadConfig(ctx);
...
export function createProposal(ctx: CliContext, input: Record<string, unknown>) {
  validateFields(input, ['name', 'goal', 'rationale', 'points', 'scope', 'phases']);
...
export function readContextCommand(ctx: CliContext, taskId: string, section: string, includeReviews: boolean) {
  ...
  const content = readFile(join(root, 'context.md'));
...
export function updateContextCommand(ctx: CliContext, payload: Record<string, unknown>) {
  ...
  const title = titleFromHeading(original).replace(/ - 上下文$/, '');
  ...
  appendLedger(ctx, 'legion_update_context', taskId, title, summarizeAuditFields(Object.keys(payload)), 'success');
  return { taskId };
}
...
export function listReviews(ctx: CliContext, taskId: string, status: string, type: string) {
  const files = ['plan.md', 'context.md', 'tasks.md'] as const;
...
export function respondReview(ctx: CliContext, payload: Record<string, unknown>) {
  ...
  if (!['plan.md', 'context.md', 'tasks.md'].includes(file)) {
...
export function generateDashboard(ctx: CliContext, taskId: string, format: 'markdown' | 'html', sections: string[], outputPath?: string) {
  ...
  const contextContent = readFile(join(taskRoot(ctx, taskId), 'context.md'));
...
function prepareTaskDraft(ctx: CliContext, input: Record<string, unknown>, fromProposal: Proposal | undefined, config: LegionConfig): TaskDraft {
  ...
  return {
    ...
    plan: renderPlan({
      title: name,
      goal,
      problem: fromProposal?.rationale ?? '待补充问题陈述。',
      acceptance: [
        'CLI/流程能够在当前范围内完成约定交付。',
        '相关文档与脚本入口保持一致。',
      ],
      assumptions: [
        { label: '假设', value: '若无额外说明，则沿用当前仓库约定与工作语言。' },
        { label: '约束', value: '实现必须严格遵守任务 Scope。' },
        { label: '风险', value: '如脚本/文档入口不一致，可能导致工作流漂移。' },
      ],
      points,
      scope,
      designIndex: fromProposal ? '.legion/tasks/<task-id>/docs/rfc.md' : '（暂无）',
      designSummary: [
...
function writeTaskDraft(draft: TaskDraft) {
  ...
  writeTextAtomic(draft.ctx, join(draft.root, 'context.md'), draft.context);
...
function parseStableReviewId(reviewId: string): { file: string; line: number } | null {
  const match = reviewId.match(/^(plan\.md|context\.md|tasks\.md):L(\d+)$/);
