...
## 4.5 Subagent 派生是强制流程，不是提示语

- orchestrator 必须按阶段派生 subagents；不要把设计、实现、测试、review、报告全部内化为单 agent 行为。
- 具体派生顺序、触发条件、输出路径只认 `SUBAGENT_DISPATCH_MATRIX.md`。
- command 入口只能选择模式（如 default / impl / rfc-heavy），不能再定义另一套 dispatch 真源。
