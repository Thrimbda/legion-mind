...
2. `brainstorm` - task contract 不稳定、需要新建任务、或需要重构 `plan.md` / `tasks.md`
3. `legion-docs` - 决定 `.legion` 文档应该写到哪里、写到什么密度
4. `spec-rfc` / `review-rfc` - Medium / High 风险任务需要设计产物时
5. `engineer` - 只有在稳定 contract 和所需 design gate 之后
6. `run-tests` / `review-code` / `review-security`
7. `report-walkthrough`
