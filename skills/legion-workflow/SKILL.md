...
- PR-backed task 的仓库证据在当前 delivery PR terminal 前收敛为 `delivery-ready`；terminal 后的 merge/checks/cleanup/refresh 或 publish/deploy 事实只进入外部 lifecycle 与最终交接，不再为状态收口反写 task/wiki。
...
进入 `git-worktree-pr` 后，默认完成 commit、push 前 fetch/rebase、push、当前 squash PR、auto-merge 尝试、checks/review、merge/closed/confirmed-abandoned 终态、worktree cleanup 与主工作区刷新；当前 PR open 时继续更新同一 branch/PR，用户沉默不是停止条件。禁止直接提交或 push `master/main`。

PR terminal 后的 merge/checks/cleanup/refresh/publish/deploy 事实进入 external-only 记录：不得自动新建或修改 branch、写 task/log/wiki/report、commit/push 或创建 closeout/publish-result/deploy-result/wiki-only PR 来追写这些状态；只做外部 publish/deploy、只读验证、worktree cleanup、refresh 与最终报告。纯外部动作可重试；若修复或回滚确需仓库改动，停止当前自动 lifecycle、向用户报告并等待明确授权，授权后的交付可以使用新的 branch/PR。

只有适用阶段证据、delivery/wiki disposition 已满足，且当前 PR 到终态、review/checks 已处理、worktree 已删除、主工作区已刷新，才可在外部 lifecycle 中声明 done。repo 内 `delivery-ready` 不等于 delivery terminal，也不要求 terminal 后改写为 `completed`。`summary`/`no-change` 是明确判定，不要求占位 walkthrough/Wiki。PR created、blocked handoff、保留 worktree或跳过刷新都不是完成。
