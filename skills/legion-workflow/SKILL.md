...
对会修改仓库文件的开发任务，Git / PR lifecycle 由 `git-worktree-pr` 作为强制外壳承载。它包裹本 skill 选择的执行模式，不改变入口门、阶段链或真源边界，也不是第四种执行模式。

## 输出语言与文档产物

- 默认用中文回答入口判断、恢复摘要、阶段路由、阻塞原因和收口交接。
- 由本 workflow 创建、更新或要求产出的 task docs、RFC/review/verification/walkthrough/wiki writeback、PR body 摘要等文档产物默认使用中文。
- skill 名称、运行模式名、路径、命令、Git/PR/check 字段、错误原文和第三方术语保持原文，不因中文化破坏真源语义。
...
`git-worktree-pr` 是仓库 lifecycle envelope：修改型开发任务的三种执行模式在 worktree/PR 外壳内运行。它不增加执行模式，不改变阶段链；重型仅设计模式若以文档 PR 交付，也只是套用 delivery envelope，不因此追加 `verify-change` / `review-change`。
...
终止条件不是“代码已改完”或“测试已过”，而是适用阶段链已产出证据，并完成 `legion-wiki` 写回后到达 `done`。阶段输出必须是可审阅证据（task docs、RFC、验证记录、review 结论、walkthrough、wiki writeback），不能只用聊天声明替代。对 PR-backed 开发任务，`done` 还要求 PR 已 merged，或已 closed / confirmed abandoned 且原因和下一步已记录；后续 review/checks 已处理；worktree 已删除；主工作区已完成 baseline refresh。blocked handoff、保留 worktree 或跳过 refresh 不等于 done。
