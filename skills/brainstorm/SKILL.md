...
2. 每次只问一个最影响设计的问题，优先收敛目标、验收、边界和外部依赖；若任务包含多个独立交付，先拆成用户明确授权、各有独立 `taskId` 的 task。修改型 task 的 contract 必须保留 `0..1` PR 配额，不能把同一 task 拆成实现 PR 加 closeout/follow-up PR。
