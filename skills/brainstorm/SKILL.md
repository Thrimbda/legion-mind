...
2. 每次只问一个最影响设计的问题，优先收敛目标、验收、边界和外部依赖；若任务包含多个独立交付，先拆成用户明确授权、各有独立 `taskId` 的 task。修改型 task 的 contract 必须禁止为了写回 terminal 状态而自动创建 closeout/follow-up PR，同时保留用户明确授权后继续仓库交付或使用新 PR 的能力。
